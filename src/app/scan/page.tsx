...
export default function ScanPage() {
    ...
    const handleMarkAttendance = async (staff: any) => {
        ...
        if (checkError) {
            let userFriendlyError = checkError.message;
            if (checkError.message.toLowerCase().includes("failed to fetch")) {
                userFriendlyError = "Connection Error: Could not connect to the database. Please check your internet or try again in a few minutes.";
            }
            setError("Failed to verify records: " + userFriendlyError);
            setLoading(false);
            return;
        }
                ...
                {staffRecords.length === 0 ? (
                    ...
                    <div key={staff.id} className={styles.staffCard} onClick={() => handleMarkAttendance(staff)}>
                        <div className={styles.photoContainer}>
                            <img 
                                src={staff.photo_url || "https://i.pravatar.cc/150?u=none"} 
                                alt={staff.name} 
